...
class HH(Vacancy, APIManager):
    ...
    def load_vacancy(self):
        """Проходим циклом по словарю, берем только нужные нам данные и записываем их в переменную 'vacancies' """
...
class Super_job(Vacancy, APIManager):
    ...
    def load_vacancy(self):
        """Проходим циклом по словарю, берем только нужные нам данные и записываем их в переменную 'vacancy_list_SJ' """
        data = self.get_vacancies()
        vacancy_list_SJ = []
...
def job_vacancy():
    ...
    website = input('введите платформу для поиска: (1 - HH, 2 - SJ, 3 - обе платформы)  ')
